    ...
    union() {
      for ( i = [1:Size_barb_count]) {
        translate([0, 0, (i - 1) * Size * 0.9])
          cylinder( h = Size , r2 = Size * 0.85 / 2, r1 = Size * 1.16 / 2, $fa = 0.5, $fs = 0.5 );
      }
    }
    translate([0, 0, -0.1]) cylinder(h = Size * Size_barb_count , d = Size_Bore, $fa = 0.5, $fs = 0.5 );
  }
}
